...
class cItems{
  ...
  public function consultar($ord,$dir){ 
   ...
   if($con->conectar()==true){
		$query = "select * from ua_items ".$query1;
		//echo $query; exit();
  ...
  public function agregar($nombre,$apellidop,$apellidom,$rut,$clave,$email){ 
   ...
   if($con->conectar()==true){
		 ...
		 $query1 = "insert into ua_items (nombre) values ('$nombre')";
		//echo $query1;exit(); 		
  ...
  public function modificar($id, $nombre){   
   $con = new Conexion;
   if($con->conectar()==true){
		$query = "update ua_items set nombre='$nombre' where id='$id'";	
		//echo $query; exit();
			if(!($result = @mysql_query($query))){
  ...
  public function eliminar($id){ 
   $con = new Conexion;
   if($con->conectar()==true){
		$query = "delete from ua_items where id='$id'";
			if(!($result = @mysql_query($query))){
